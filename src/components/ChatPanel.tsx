...
export function ChatPanel({ activeWidget }: ChatPanelProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col h-[600px]">
      ...
      {/* Input */}
      <form onSubmit={sendMessage} className={`px-4 pb-4 ${activeWidget && showSuggestions && messages.length <= 1 && suggestedPrompts[activeWidget] ? 'pt-0' : 'pt-4 border-t border-gray-200 dark:border-gray-700'} border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800`}>
        {!activeWidget ? (
